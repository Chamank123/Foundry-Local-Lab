...
def load_nn(path, weight=1.0, ep_policy="legal"):
    ...
    if not np.isfinite(weight) or not 0 <= weight <= 1:
        raise ValueError("weight must lie between 0 and 1")
    if ep_policy not in ("legal", "raw"):
        raise ValueError("ep_policy must be legal or raw")
    with np.load(path, allow_pickle=False) as data:
        kind = str(data["meta_evaluator_kind"].item()) if "meta_evaluator_kind" in data else "full"
        if kind not in ("full", "hc_residual_v1"):
            raise ValueError("Unsupported evaluator kind: " + kind)
        if kind == "hc_residual_v1" and ep_policy != "legal":
            raise ValueError("Residual checkpoint requires legal EP policy")
    import nn_model
    ...
    # 0=original HC; 1=legacy full NN; 2=legal-EP full NN;
    # 1000..2000=legal-EP full-NN blend; 3000..4000=HC + residual.
    amount = int(round(weight * 1000))
    if amount == 0:
        _USE_NN[0] = 0
    elif kind == "hc_residual_v1":
        _USE_NN[0] = 3000 + amount
    elif ep_policy == "raw":
        if amount != 1000:
            raise ValueError("Raw policy is only supported for the legacy full-NN control")
        _USE_NN[0] = 1
    else:
        _USE_NN[0] = 2 if amount == 1000 else 1000 + amount
    _warmup_nn()
    new_game()
    return _NNW
...
def evaluate(bd, nnw, use_nn):
    ...
    hc = evaluate_handcrafted(bd)
    if use_nn == 0:
        return hc
    neural = score_nn(bd, nnw, True)
    if use_nn == 2:
        result = neural
    elif use_nn >= 3000:
        result = hc + ((use_nn - 3000) / 1000.0) * neural
    else:
        alpha = (use_nn - 1000) / 1000.0
        result = (1.0 - alpha) * hc + alpha * neural
    return int(max(-6000.0, min(6000.0, result)))


@njit(cache=False)
def evaluate_handcrafted(bd):
    """Preserved handcrafted function; math is unchanged from the supplied engine."""
    mg = 0
    eg = 0
    phase = 0
    for pt in range(6):
